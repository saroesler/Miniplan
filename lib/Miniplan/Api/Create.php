...
class Miniplan_Api_Create extends Zikula_AbstractApi
{
	...
	public function getRoutinedescription($args)
	{
		...
		$description[1] = array ( "id" => 1, "text" =>$this->__("The Vuong- Routine chooses a voluntary acolyte for every worship at first. If there isn't any voluntary acolyte it chooses an other acolyte by a random routine. After chosing an acolyte it tests, if the acolyte is not devided in the last x days or the last x worships. You can set the time by the settings. If there are not enought ministrants for this worship, the routine select ones that are located too close. The random routine selects the acolyte having the lowest unber of devidings. <br/>After that it looks, if there is a prtner. If the partner can't ministrate at this worship, both (partner and acolyte) are removed. If the partner is too for this worship and partner order is longer than two, the partner will be not devided (This works, if the partner state is 'have to', too). otherwise they both won't be devided, too. If there isn't anything of this, the partner will devided. The 'like'- option for partner says only, that the partner is tested for this worship next. If He cant, the first ministrant will devidet, too.<br/> The routine don't deals with devidings to churches or dates."),
	...
	**/
	...
	public function Vuong()
	{
		...
		**/
		...
		{
			$alternativ = array();		//minis, die im Notfall eingeteilt werden können.
			$wid = $worship->getId();
			...
			while($worship->needMini() )
			{
				...
				while($first_item !=1)
				{
					...
					if(($minis_array[$my_mid]->calendar($wid) == 2)&&($minis_array[$my_mid]->eingeteilt() <= ($statistik->getDurchschnitt()+2))&&($minis_array[$my_mid]-> abstand($plan, $worship_index, $logManager, $this->getVars()) < 0))
					{
				...
				if(!$voluntary_state)
				{
					...
					***********************************/
					...
					while($my_reverence < $this->getVar('voungMaxLevel'))
					{
						...
						while($first_item <=0)
						{
							...
							if($minis_array[$minis_rand_array[$my_mini_index]]->eingeteilt() == $my_reverence)
							{
								...
								//mini kann
								{
									...
									$my_reverence = $this->getVar('voungMaxLevel');
									...
									if($distance > 0){
										...
										foreach($alternativ as $item){
											if($item['mid'] == $my_mini_index){
												$hasit = 1;
												break;
											}
										}
										...
										if(!$hasit)
											$alternativ[] = array('mid' => $my_mini_index, 'distance' => $distance);
									}
								}
							}
							$my_mini_index++;
					...
					if($first_item == 1)
					{
						if(count($alternativ) < $worship->needMini() ){
							$logManager->add((str_pad(__LINE__, 3, 0, STR_PAD_LEFT)),"  Es können nicht genügend Ministranten für den Gottesdienst ". $worship->getId() ." eingeteilt werden!!! (nur ". count($alternativ) . " von " . $worship->needMini() . "verfügbar)\n Break\n");
							$error_warnings.= "Es können nicht genügend Ministranten für den Gottesdienst ". $worship->getId() ." eingeteilt werden!!!\n Break\n";
							$logManager->close((str_pad(__LINE__, 3, 0, STR_PAD_LEFT)));
							...
							//Notfallministranten einteilen
							
							$distancesort = array();

							foreach ($alternativ as $item)
							{
								$distancesort[]  = $item['distance'];
							}


							array_multisort($distancesort, SORT_DESC, $alternativ);
