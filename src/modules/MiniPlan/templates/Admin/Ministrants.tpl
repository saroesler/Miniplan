...
{include file='Admin/Header.tpl' __title='Ministrants' icon='display'}

<form class="z-form" method="post" action="{modurl modname='MiniPlan' type='Admin' func='ministrantsManage'}">
		<table class="z-datatable">
			...
			<tbody>
				{foreach from=$ministrants item='ministrant'}
					<tr>
						<td>{$ministrant->getWid()}</td>
						<td>{$ministrant->getCname()}</td>
						<td>{$ministrant->getDateFormatted()}</td>
						<td>{$ministrant->getMinistrantsRequired()}</td>
